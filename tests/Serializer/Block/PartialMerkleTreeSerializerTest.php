...
class PartialMerkleTreeSerializerTest extends AbstractTestCase
{
    public function testSerialize()
    {
        ...
        $serialized = $tree->getBuffer();
        $serializer = new PartialMerkleTreeSerializer();
        $parsed = $serializer->parse($serialized);

        $treeBits = $tree->getFlagBits();
        $parsedBits = $tree->getFlagBits();
        $last = min(count($treeBits), count($parsedBits));
        for ($i = 0; $i < $last; $i++) {
            $this->assertEquals($treeBits[$i], $parsedBits[$i]);
        }

        $this->assertEquals($tree->getHashes(), $parsed->getHashes());
        $this->assertEquals($tree->getBuffer(), $parsed->getBuffer());

        $treeMatch = [];
        $tree->extractMatches($treeMatch);

        $parsedMatch = [];
        $parsed->extractMatches($parsedMatch);

        $this->assertEquals($treeMatch, $parsedMatch);
    }
}
